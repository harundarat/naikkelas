...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { message, image, chatId: incomingChatId, force_image, isTemporary } = body;
    ...
    const chatHistory = incomingChatId ? await db.query.messages.findMany({
      where: and(eq(messages.chatId, incomingChatId), eq(messages.userId, user.id)),
    ...
    const imageGenerationKeywords = ['generate image', 'create a picture of', 'draw'];
    const isImageGenerationRequest = force_image || imageGenerationKeywords.some((kw: string) => message.toLowerCase().includes(kw));
    ...
    if (isTemporary) {
      ...
      if (isImageGenerationRequest || image) {
        ...
        if (formattedHistory.length <= 1) {
          logger.info('No significant history, using user message directly as prompt.');
          finalPrompt = message;
        } else {
          const briefResponse = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL}/api/generate-creative-brief`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ history: formattedHistory }),
          });
          if (!briefResponse.ok) throw new Error('Failed to generate creative brief');
          const { summarizedPrompt } = await briefResponse.json();
          finalPrompt = summarizedPrompt;
        }

        const imageParts = formattedHistory.flatMap((msg: any) => msg.parts.filter((part: any) => part.inlineData));
        if (image) {
          imageParts.push(await fileToGenerativePart(image));
        }

        const responseStream = await model.generateContentStream([finalPrompt, ...imageParts]);
        ...
        return NextResponse.json({ ...accumulatedResponse, timestamp: new Date().toISOString() });
      }
    ...
    if (creditRecord.credits < MINIMUM_CREDITS_THRESHOLD) {
      logger.warn('User has insufficient credits', { userId: user.id, credits: creditRecord.credits, minimumRequired: MINIMUM_CREDITS_THRESHOLD });
      return NextResponse.json(
    ...
    try {
      await db.insert(messages).values({
        ...
        role: 'user',
        content: message,
        attachments: image ? [{ type: image.split(";")[0].split(":")[1], url: image }] : [],
      });
      logger.info('User message saved to DB', { chatId: currentChatId, userId: user.id });
    } catch (dbError) {
      logger.error('Failed to save user message to DB', { error: dbError, chatId: currentChatId });
      throw dbError;
    }
    ...
    if (isImageGenerationRequest || image) {
      ...
      const model = genAI.getGenerativeModel({ model: "gemini-3-pro-image-preview" }); // No system instruction for image gen model usually
      const responseStream = await model.generateContentStream([finalPrompt, ...imageParts]);
      ...
      const outputTokens = aggregatedResponse.usageMetadata?.candidatesTokenCount || 0;
      logger.info('Image generation token usage', { outputTokens });

      // --- NEW: Upload Generated Image to Supabase Storage ---
      if (accumulatedResponse.imageUrl && accumulatedResponse.imageUrl.startsWith('data:')) {
        try {
          const base64Data = accumulatedResponse.imageUrl.split(',')[1];
          const mimeType = accumulatedResponse.imageUrl.split(';')[0].split(':')[1];
          const buffer = Buffer.from(base64Data, 'base64');
          const fileExtension = mimeType.split('/')[1] || 'png';
          const fileName = `generated-${Date.now()}-${Math.random().toString(36).substring(7)}.${fileExtension}`;

          logger.info(`Uploading generated image to Supabase Storage as ${fileName}...`);
          ...
          logger.error("Failed to upload generated image to Supabase Storage:", uploadError);
          // Fallback is to keep the base64 string in accumulatedResponse.imageUrl
        }
      }

      logger.info('Attempting to save AI image response to DB...', { chatId: currentChatId });
      try {
        await db.insert(messages).values({
          id: `msg_${Date.now() + 1}`,
          chatId: currentChatId,
          userId: user.id,
          role: 'ai',
          content: accumulatedResponse.reply || `Generated image for: ${message}`,
          attachments: accumulatedResponse.imageUrl ? [{ type: 'image/png', url: accumulatedResponse.imageUrl }] : [],
        });
        logger.info('AI image response saved successfully', { chatId: currentChatId });
      } catch (dbError) {
        logger.error('Failed to save AI image response to DB', { error: dbError, chatId: currentChatId });
      }

      // Deduct tokens after successful response
      await deductTokens(user.id, outputTokens);

      return NextResponse.json({ ...accumulatedResponse, chatId: currentChatId, tokensUsed: outputTokens, timestamp: new Date().toISOString() });
    }
    ...
    logger.info('Attempting to save AI text response to DB...', { chatId: currentChatId });
    try {
      await db.insert(messages).values({
        id: `msg_${Date.now() + 1}`,
        chatId: currentChatId,
      ...
      logger.info('AI text response saved successfully', { chatId: currentChatId });
    } catch (dbError) {
      logger.error('Failed to save AI text response to DB', { error: dbError, chatId: currentChatId });
    }
